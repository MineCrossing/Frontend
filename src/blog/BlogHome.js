...
const BlogHome = props => {
    ...
    const createBlogButton = props.auth?.admin ?? false ? <Button variant={"contained"} color={"secondary"} href={"/createblog"}>Create New</Button> : "";
    return (
        <main className={classes.blogHome}>
            ...
            <section className={classes.blogHomeContent}>
                ...
                <section className={classes.blogHomeBlogContainer}>
                    ...
                    <span className={"separator"}> </span>
                    <PreviewBlogsList remove={(id) => setBlogs(blogs.filter(b => b.blogPostID !== id))} auth={props.auth} blogs={blogs?.filter(b => getBlogDate(b?.date).getMonth() === month && getBlogDate(b?.date).getFullYear() === year)}/>
                </section>
            </section>
        </main>
    );
